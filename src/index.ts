...
const SESS = new KVStore("apex-sessions");
const DEALS = new KVStore("apex-deals");
const ASSESS = new KVStore("apex-assess");
const USAGE = new KVStore("apex-usage");
const SECRETS = new SecretStore("apex-secrets");
...
const HTML = (s: string) =>
  ...
  <title>Apex</title><style>
    body{font-family:Inter,system-ui,sans-serif;margin:24px;max-width:900px}
    .btn{padding:10px 14px;border:1px solid #ddd;border-radius:12px;background:#fff}
    ...
    a{color:#111} hr{margin:18px 0}
  </style>${s}`, { headers: { "content-type": "text/html; charset=utf-8" } });
...
async function getSession(req: Request) {
  ...
  if (!id) return { id:"", user:null as null|{ sub:string; email?:string } };
...
async function listDeals(userId: string) {
  ...
  do {
    ...
    if (list) keys.push(...list); cursor = c;
  } while (cursor);
  const out = [];
  for (const k of keys) { const e = await DEALS.get(k); if (e) out.push(await e.json()); }
  out.sort((a,b)=> (b.updatedAt||0)-(a.updatedAt||0));
  return out;
}

function safeJSON(maybe: string) {
  let s = maybe.trim().replace(/^```json|^```/i,"").replace(/```$/,"");
  try { return JSON.parse(s); } catch { return { tier:"Workable", go_hold_nogo:"Hold", total_score:60, analysis:"Fallback." }; }
}
async function llmAssess(input: any) {
  const provider = (await (await SECRETS.get("LLM_PROVIDER"))?.text()) || "anthropic";
  const model = (await (await SECRETS.get("LLM_MODEL"))?.text()) || "claude-3-5-sonnet-20240620";
  const prompt = `Return strict JSON only with fields:
{"tier":"Strong|Workable|Weak","go_hold_nogo":"Go|Hold|No-go","total_score":0,"analysis":"..."}
Deal input: ${JSON.stringify(input)}`;

  if (provider === "openai") {
    const key = await (await SECRETS.get("OPENAI_API_KEY"))?.text();
    ...
    const key = await (await SECRETS.get("ANTHROPIC_API_KEY"))?.text();
...
async function handle(req: Request): Promise<Response> {
  ...
  if (req.method === "POST" && p === "/assess") {
    ...
    deal.lastAssessmentId = ts; deal.lastTier = result.tier; deal.lastScore = result.total_score; deal.updatedAt = ts;
    await DEALS.put(`deal:${user.sub}:${dealId}`, JSON.stringify(deal));

    const ym = new Date().toISOString().slice(0,7).replace("-","");
    const key = `use:${ym}`;
    let count = 0; const cur = await USAGE.get(key); if (cur) count = Number(await cur.text());
    await USAGE.put(key, String(count+1));
  ...
  if (req.method === "GET" && p === "/health") return new Response('{"ok":true}', { headers:{ "content-type":"application/json" }});
